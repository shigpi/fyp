...
from app.schemas.plan import PlanResponse, AdminPlanCreate, AdminPlanUpdate
from app.schemas.subscription import (
    SubscriptionResponse,
    AdminSubscriptionCreate,
    AdminSubscriptionUpdate,
)
from app.schemas.subscription_usage import (
...
from app.services.security.service import security_service

router = APIRouter()


# ── Batch lookup helper (Lambda optimization — avoids N+1 queries) ─────────
def _build_lookup(db: Session, model_class, ids, key="id"):
    """Batch-fetch rows by IDs, return {id: row} dict."""
    if not ids:
        return {}
    rows = db.query(model_class).filter(getattr(model_class, key).in_(ids)).all()
...
def read_organizations(
    ...
    owners = _build_lookup(db, User, owner_ids)
    results = []
    for org in orgs:
        data = OrganizationResponse.model_validate(org)
        owner = owners.get(org.owner_id)
...
def read_subscriptions(
    ...
    orgs = _build_lookup(db, Organization, org_ids)
    plans = _build_lookup(db, Plan, plan_ids)
    results = []
    for sub in subs:
        data = SubscriptionResponse.model_validate(sub)
        org = orgs.get(sub.org_id)
...
def read_org_members(
    ...
    orgs = _build_lookup(db, Organization, org_ids)
    users = _build_lookup(db, User, user_ids)
    results = []
    for m in members:
...
def read_subscription_usages(
    ...
    org_ids = {u.org_id for u in usages}
    sub_ids = {u.subscription_id for u in usages}
    orgs = _build_lookup(db, Organization, org_ids)
    subs_map = _build_lookup(db, Subscription, sub_ids)
    plan_ids = {s.plan_id for s in subs_map.values()}
    plans = _build_lookup(db, Plan, plan_ids)
    results = []
    for u in usages:
        data = SubscriptionUsageResponse.model_validate(u)
        org = orgs.get(u.org_id)
        sub = subs_map.get(u.subscription_id)
        data.org_name = org.name if org else None
        if sub:
            plan = plans.get(sub.plan_id)
            plan_name = plan.name if plan else str(sub.plan_id)
...
def search_users(
    ...
    # Memberships involving matched users — batch lookups
    members = []
    if user_ids:
        members_q = db.query(OrgMember).filter(OrgMember.user_id.in_(user_ids)).all()
        m_org_ids = {m.org_id for m in members_q}
        m_user_ids = {m.user_id for m in members_q}
        m_orgs = _build_lookup(db, Organization, m_org_ids)
        m_users = _build_lookup(db, User, m_user_ids)
        for m in members_q:
            o = m_orgs.get(m.org_id)
            u = m_users.get(m.user_id)
            members.append({
                "id": m.id,
                "org_id": m.org_id,
    ...
    # Subscriptions for matched orgs — batch lookups
    subs = db.query(Subscription).filter(Subscription.org_id.in_(org_ids)).all() if org_ids else []
    sub_ids = [s.id for s in subs]
    s_plan_ids = {s.plan_id for s in subs}
    s_plans = _build_lookup(db, Plan, s_plan_ids)
    org_lookup = {o.id: o for o in orgs}  # reuse already-fetched orgs
    subs_data = []
    for s in subs:
        plan = s_plans.get(s.plan_id)
        org = org_lookup.get(s.org_id)
        subs_data.append({
            "id": s.id,
            "org_id": s.org_id,
    ...
    # Usage records for matched orgs — batch lookups
    usages = db.query(SubscriptionUsage).filter(SubscriptionUsage.org_id.in_(org_ids)).all() if org_ids else []
    u_sub_ids = {u.subscription_id for u in usages}
    u_subs = _build_lookup(db, Subscription, u_sub_ids)
    u_plan_ids = {s.plan_id for s in u_subs.values()}
    u_plans = _build_lookup(db, Plan, u_plan_ids)
    usages_data = []
    for u in usages:
        org = org_lookup.get(u.org_id)
        sub = u_subs.get(u.subscription_id)
        plan = u_plans.get(sub.plan_id) if sub else None
        usages_data.append({
            "id": u.id,
            "org_id": u.org_id,
            "org_name": org.name if org else None,
