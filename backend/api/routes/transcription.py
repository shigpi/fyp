...
@router.post("")
async def transcribe_audio(file: UploadFile = File(...), language: str = None, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    """
    ...
    subscription = db.query(Subscription.plan_id, Subscription.id).filter(Subscription.org_id == membership.org_id, Subscription.current_period_end >= date.today()).first()
    if not subscription:
